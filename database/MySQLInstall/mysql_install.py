...
def CheckReplaceFile(configure_dic):
    ...
    if os.path.exists(data_file_path):
        msg = "The MySQL Instance Port: %s exists, Pleace Change" % configure_dic['--instance-ports']
        print LINE
        print ERROR % msg
        print LINE
        sys.exit()
...
def InstallMysql(install_cfg):
    ...
    if os.path.exists(install_cfg['--base-prefix']) and os.path.exists(os.path.join(install_dir, mysql_src_name)):
        ...
        if os.path.exists(mysql_src_tar):
            ...
            msg = "%s is not exist, pls check it." % mysql_src_tar
            print LINE
            print ERROR % msg
            print LINE
            sys.exit()
    ...
    mysql_value = commands.getstatusoutput("grep -w mysql%s /etc/profile" % install_cfg['--base-prefix'])
    if len(mysql_value[1]) == 0:
        commands.getstatusoutput("echo alias mysql%s='\"'%s/bin/mysql --defaults-file=%s'\"' >> /etc/profile" % (install_cfg['--instance-ports'], install_cfg['--base-prefix'], mysql_conf_file))
    ...
    if int(mysql_initialize[0]) != 0:
        ...
        msg = "Failed to initialize MySQL data directory. Port: %s" % install_cfg['--instance-ports']
        print LINE
        print ERROR % msg
        print LINE
        sys.exit()
    ...
    if int(cfg_mysql_privileges[0]) != 0:
        ...
        msg = "Failed to set MySQL root password! Please do it manually."
        print LINE
    ...
    if int(check_mysql[0]) != 0:
        msg = "Failed to connect to port %s! You need to create MySQL monitoring user manually." % install_cfg['--instance-ports']
        print LINE
    ...
    if int(install_rpl_semi_sync[0]) != 0:
        ...
        msg = "Install rpl_semi_sync failed on %s" % install_cfg['--instance-ports']
        print LINE
        ...
        if check_result['result_state'] == "false":
            ...
            if create_file_result['result_state'] == "true":
                ...
                if install_result == "true":
                    install_finish_message = "Install MySQL Finish"
                    print LINE
